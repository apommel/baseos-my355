...
- Format matched exactly to the vendor logo: **640×480, 24-bit, uncompressed,
  bottom-up BMP** (921,654 bytes).
- Generated by `tools/make-bootlogo.sh` (renders `fbsplash 0` → PPM → BMP), vendored
  at `assets/bootlogo.bmp`. **Regenerate it whenever the splash design or font
  changes** so the bootlogo stays in step.
- `build-image.sh` writes it onto p2 with `mcopy -o` after copying the boot-prefix, so
  the vendored `boot-prefix.img` stays pristine. (This is our own image — writing p2
