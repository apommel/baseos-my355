...
docker run --rm --platform linux/arm64 -v "$WORK":/work alpine:3.20 sh -euc '
  apk add -q qemu-system-aarch64 linux-virt
  R=/tmp/r; mkdir -p "$R"
  tar -xf /work/rootfs.tar -C "$R"
  echo "::once:/bin/sh -c \"echo BASEOS-USERSPACE-BOOT-OK > /dev/console\"" >> "$R/etc/inittab"
  ...
  if grep -aq "BASEOS-USERSPACE-BOOT-OK" /tmp/boot.log; then
    echo "RESULT: PASS — busybox init + inittab + rcS completed"
  else
    echo "RESULT: FAIL — marker not reached"; tail -20 /tmp/boot.log
  fi
'
