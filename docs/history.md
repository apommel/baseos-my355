...
| 2026-09-21 | **Kernel compression re-benchmarked** at the 23.8 MB/s read and 1800 MHz decode: 74 encodings, every format `bootm` takes, U-Boot's own decoders on the device. zstd stays; the lever is building its decoder at `-O2` rather than `-Os` (patch `0007`), 13% faster for 10 KB. lz4 comes within 10 ms once `-mstrict-align` and byte-wise `memcpy` stop crippling it (328 → 79 ms), but is 2.9 MB larger. `bootm_load_os` 224 → **194 ms**, `start_kernel` 971 → **940 ms** over three warm reboots ([U-Boot](uboot.md)) |
| 2026-09-21 | **Boot card lost on resume from deep suspend.** Now and then, after a NextUI suspend, the UI ran on from RAM while adb, ssh and any game launch failed, until a long press. Reproduced in 7 cycles, twice, by a loop suspending on the RTC alarm with its log on the other card: `vccio_sd: Restricting voltage, 2700000-1950000uV`, `mmc1: error -5 doing runtime resume`, `card 1234 removed`. Both slots' controllers name `vccio_sd` as `vqmmc-supply` in the vendor tree, and on resume both cards init at once: the left slot's 3.3 V request made the boot card's 1.8 V switch unsatisfiable. The 2026-09-16 retraction holds for the left slot's pins, not for its controller. Its `vqmmc-supply` is removed (`rkbootimg.detach_sd_slot1_vqmmc`); ~75 cycles clean after. Not U-Boot, and not SDR104: SDR25 needs the same switch. Found with it: `pstore` and `panic=10` so a panic leaves a record, and `usb-gadget-adb` rebinding the UDC after `adbd` reopens functionfs ([card](card.md), [diagnostics](diagnostics.md)) |
...
| 2026-09-22 | **Charge mode and the gauge, after review against the vendor's `charge_animation.c` and `fg_rk817.c`.** A **low-battery guard** again: on a cold start, `BAT_VOL` and `PWRON_VOL` both under the vendor tree's 3350 + 50 mV power off, with three blinks of the charge LED, or start charge mode with a charger in, where the power key waits for 3400 mV. Both readings must agree, since either can be stale and a false alarm would stop the unit booting. Charge mode no longer gives up after 60 s of a paused charge — the PMIC would resume it with the LED dark and only the decaying counter to account for it — and it boots on a **2 s press**, the vendor's `KEY_LONG_DOWN_MS`, rather than any touch. Leaving it, the gauge takes any change of the counter, which was running. The fuel gauge reads each register in one transfer: verified against byte reads on this unit, where `BAT_VOL` converted as the kernel does read 4182 mV against its `voltage_now` of 4182000 µV. Its decision is now `rk817_fg_decide()`, compiled by `tests/test-fuel-gauge.sh` as it stands rather than restated ([U-Boot](uboot.md)) |
| 2026-09-22 | **`BAT_VOL` read stale in U-Boot**: 3926 mV on the first cold boot of that build, on a battery at 99% that the kernel read at 4086 mV. The kernel sets the gauge's voltage output to an average (`GG_CON` bit 1 = 0) at every probe, and the average outlives the power-off; the vendor U-Boot switches it to single samples before reading. `my355 charge` now does too, so charge mode's power-key check, which reads `BAT_VOL` alone, sees the live voltage ([U-Boot](uboot.md)) |
| 2026-09-22 | **Charge mode charged at ~340 mA**: 91 → 204 mAh in about 20 minutes off the charger. The RK817 powers on with a 450 mA input limit, and `my355 charge` left it there while the SoC drew its own ~150 mA; the kernel programs 1500 mA with a 4.5 V input floor at probe (`usb_ctrl` 0xdb, `chrg_out` 0xa3 read back from a running unit), and the vendor U-Boot sets the same input pair, 450 mA only when its USB detection calls the port a plain one. Charge mode now writes that pair and leaves the charge current register alone, as the vendor does ([U-Boot](uboot.md)) |
| 2026-09-24 | **0.7.0 powered off every boot on a user's Flip** whose RK8600 does not answer on i2c0 (stock too: `rk860-regulator 0-0040: Failed to get chip ID!`, no cpufreq). `my355 cpu` refused every rate and the return to 1104 MHz stopped the boot. The vendor U-Boot never drives that regulator and sets 1104 MHz blind; `my355 cpu` now does the same when no RK8600 answers, refusing anything faster ([U-Boot](uboot.md)) |
| 2026-09-24 | **The root is mounted read-only.** `ro` replaces `rw` on the command line, and `rcS` no longer remounts `/` (`noatime`, which would also have made it writable again). Nothing on the card wrote to it any more; `/root` moved to `/data/root`. A power-off that skips `rcK` can no longer dirty the OS, and the running slot stays identical to its image ([rootfs](rootfs.md)) |
| 2026-09-24 | **`rcK` had to kill `ntpd` on short sessions and update reboots, and `S49ntp restart` left it stopped.** Not start-up in progress: `ntpd` resolves its peers once before installing its SIGTERM handler (about 15 s with no network, and a SIGTERM there kills it outright), then retries from its main loop, where musl's resolver waits out each 5 s timeout through the signal. Measured with DNS hidden: SIGTERM at 20, 25, 30 s took effect after 11.0, 6.0, 1.0 s. Separately, init reaps an orphan about once a second, so `start` saw the old `ntpd` in `pidof` and did nothing. `stop` now uses SIGKILL, `start` ignores zombies, and `rcK` stops `ntpd` first ([rootfs](rootfs.md)) |
| 2026-09-24 | **The frontend card left mounted and writable on shutdowns that NextUI did not start**, and once `/data` too. `nextui.elf` catches SIGTERM and carries on sleeping, so `rcK` kills it after 1 s; the first `umount` then fails with `EBUSY` although no process has a working directory, file or mapping on the card, and succeeds 0.15–0.2 s later. The kernel was still releasing the killed process's files. `rcK` now retries each unmount for up to 2 s (measured: 3 and 4 retries of 50 ms). The dirty flag already set stays until `fsck.fat` ([rootfs](rootfs.md)) |
| 2026-09-24 | **Holding the power key for 2 s powers off cleanly when no frontend runs.** Nothing read the key without one, so holding it, the PMIC's hard cut, was the only way off. `frontend-session` now starts `pwrkeyd` (`src/pwrkeyd.c`) when it shows `INSERT SD CARD` or `ADD FRONTEND TO SD CARD`, and stops it before handing off, since NextUI reads the key itself. It opens the PMIC's key device by name, as the lid's hall sensor reports `KEY_POWER` too, and runs `poweroff` once the key has been held for 2 s, the hold U-Boot's charge mode asks to boot, so a bump does nothing. Tested with presses injected into the key's evdev node: a 2 s hold ran `poweroff`, a 1 s press and a release alone did not, one instance lasted across respawns, and it was gone once NextUI started ([rootfs](rootfs.md)) |
| 2026-09-24 | **A dirty FAT card is repaired before it is mounted.** A card once powered off while mounted kept its dirty flag for good: Linux never clears one it found at mount, and neither does macOS's Disk Utility. `mount-frontend` now runs `fatdirty` (`src/fatdirty.c`), which reads the flag where the kernel does, and on a set flag the stock `fsck.fat -a` (dosfstools 4.2, now harvested) under `REPAIRING SD CARD`. Checked on a stock unit against loop images: a copy of a FAT32 or FAT16 volume taken while mounted reads dirty, `fsck.fat -a` clears it and keeps the files, and the kernel no longer warns at mount, while a dirty volume mounted and cleanly unmounted stays dirty. A clean check costs **1.8 ms** (200 runs). The first real repair, on a 115 GiB card with 5,704 files in the left slot, took **5.0 s**: ~29 MiB of FATs read at ~22 MB/s, then ~42 MiB of directories. `/proc/<pid>/io` is not in this kernel, so the progress bar follows the partition's sector counters instead: the FATs to 50%, the rest creeping towards 95. Upstream H700 BaseOS runs `fsck.fat` only after a failed mount ([rootfs](rootfs.md)) |
| 2026-09-24 | **Settings from `baseos.conf` on the BaseOS card**, upstream's format and parser, keeping `hostname` and `ssh_password`. The hostname defaults to `miyoo-flip` instead of `nextui`, and goes out with DHCP requests. Every unit had shared the baked root password, with SSH open on Wi-Fi. `/etc/init.d/dev` applies it in the background before `dropbear`: 12 ms with no file, 28 ms with a password (SHA-512), plus 9 ms to mount the volume read-only when the frontend is on the other card. Checked on device: `hostname = flip-test  # test` and `ssh_password=  p#ss w=rd` with CRLF gave that hostname in `/etc/hosts` and to `udhcpc`, the new password logged in and `root` was refused. Hand-off stayed at 0.75–0.76 s ([rootfs](rootfs.md)) |
| 2026-09-25 | **`<hostname>.local` over mDNS**, with upstream's static avahi-daemon (avahi 0.8, musl, no D-Bus, 526 KB), built by `build-avahi.sh` because stock has no responder, and its `baseos-mdns` DHCP hook. It first failed to start: even with `--no-drop-root`, avahi looks up an `avahi` user to own `/run/avahi-daemon`, which is why upstream carries one in `passwd`; ours now does too. Checked on device from a clean boot: the daemon started in the second the lease arrived, and `miyoo-flip.local` resolved from a Mac and took SSH. With `mdns=false` on the card, Wi-Fi came up, no daemon ran and the name did not resolve. No boot cost, with settings and mDNS both in: hand-off at 1.93 s cold and 1.91–1.93 s over five warm reboots, and `nextui.elf` at 2.43 s cold and 2.36–2.40 s warm, against 1.94 s and 2.40–2.42 s before ([rootfs](rootfs.md)) |
...
**Since done (2026-09-21):** `rcS` mounts `pstore` and keeps the records of a
boot that crashed ([diagnostics](diagnostics.md)).
