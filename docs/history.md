...
| 2026-09-20 | **The coulomb counter does not survive a power-off, and `OFF_CNT` is not minutes.** Two further offs: 4h20m cost 112 mAh where ~9.4 h cost 2,670 — no rate, so nothing to threshold or extrapolate. `my355 fg` now ignores the counter after a real off and takes `PWRON_VOL` against the cell's `ocv_table`, which is what the vendor U-Boot does (`rk817_bat_first_pwron`, and a Miyoo-added `miyoo force first boot battery` in its binary) and why stock never shows this. **Retracted:** the 40-point counter-vs-voltage threshold, and the constant ~258 mAh/h drift rate behind it — both rested on reading `OFF_CNT` as minutes. A *timed* one-hour off reads **6**: it steps ~10 minutes, which also makes the vendor kernel's `pwroff_min >= 30` about five hours. Only its being zero is used, to tell a real off from a warm reboot — where the counter is sound and `PWRON_VOL` is stale, and `ON_SOURCE` is no help because it stays `0x80` through one. 13.5 → **16 ms** ([U-Boot](uboot.md)) |
