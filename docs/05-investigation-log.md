...
| 2026-08-23 | **Stock measured end to end: power-on → NextUI frame 31.50 s**, not the ~18.5 s quoted until now. The 15.8 s hand-off was right; NextUI's own start, guessed at 2–4 s, is **15.7 s**. A stopwatch had said 28–32 s all along ([boot budget](01-boot-budget.md)) |
| 2026-08-23 | **Retracted:** "the `launch.sh` prologue is identical on stock". It is **0.89 s on BaseOS against 12.45 s on stock** — a bigger saving than the whole vendor userland we delete, from a script we neither own nor changed. Unattributed: kernel log and syslog silent throughout; I/O, `nextval.elf`, `amixer`, `modetest`, `sync` and card walks each excluded on the device ([boot budget](01-boot-budget.md)) |
| 2026-08-23 | BaseOS re-measured after Bluetooth: first frame **7.98 s** (was 7.62), hand-off **5.05 s** (was 4.91), `rcS` 0.07 → 0.21 s. The attribution given here was **wrong** — see below ([boot budget](01-boot-budget.md)) |
| 2026-08-23 | Clock method pinned: `/proc/uptime` and printk timestamps differ by a per-boot offset, measured by writing an uptime reading into `/dev/kmsg` and reading the printk timestamp back. Needed on stock, whose squashfs root has no `jbd2` anchor ([boot budget](01-boot-budget.md)) |
| 2026-08-23 | **Retracted:** "the system bus is only ~20 ms; the rest is the first writes to `/data`". The error was reading `dbus-daemon`'s `starttime` as when `rcS` reached it — `system.conf` has `<fork/>`, so it is the daemonised *child*, after the config parse and socket bind. Bracketed with two free `mark` calls instead: **`dbus-daemon --system` is 0.14 s of a 0.20 s `rcS`**; the `/data` writes are 0.01 s ([boot budget](01-boot-budget.md)) |
...
| 2026-08-23 | Preloader patch reimplemented in **BusyBox only** and run on the device: 2.2 s, output byte-identical to `tools/mkpreloader.py`, fails closed on corrupt/short/foreign/already-patched input ([SD boot](02-sd-boot.md)) |
| 2026-08-23 | Card installer packaged as a 28 KiB `miyoo355_fw.img` and driven end-to-end through stock's own gate and `miyoo_fw_update` extraction — **dry run, nothing flashed** ([SD boot](02-sd-boot.md)) |
...
| 2026-08-23 | Distribution settled: **patch in place, ship no preloader binary** — the DDR blob is paired with the unit, and more than one stock SPL build exists ([SD boot](02-sd-boot.md)) |
| 2026-08-23 | Stock automounts with usbmount: two mountpoints per slot for three filesystems, allocated by lock race. **Five trials, no reliable winner** — no correlation with slot, LBA, entry number or cache state ([SD boot](02-sd-boot.md)) |
| 2026-08-23 | **Retracted:** "on a cold boot the rootfs always wins the first mountpoint", read off two trials. A cold boot in the right slot gave it to the FAT and the rootfs none, so the installer was never found. It ships on all three filesystems ([SD boot](02-sd-boot.md)) |
| 2026-08-23 | Installer run for real off a card at boot (left slot, so stock still booted): stock's gate fired, the payload unpacked, `mtd5` read as `620648c2…`, refused as already patched, **nothing written**, no reboot ([SD boot](02-sd-boot.md)) |
| 2026-08-23 | Installer confirmed idempotent on **GammaLoader's preloader** too — its container passes all four IDB hashes and is refused on the `/pinctrl` guard, so an existing GammaLoader install is never replaced ([SD boot](02-sd-boot.md)) |
...
| 2026-08-23 | Installer extracted back out of the built card and rehearsed against the restored NAND: derived `620648c2…`, the image known to boot, and stopped before the erase ([SD boot](02-sd-boot.md)) |
...
| 2026-08-23 | **Preloader patched for the first time from a card**, unattended: `36d663ef…` → `620648c2…`, flash attempt 1, readback verified. The full path works ([SD boot](02-sd-boot.md)) |
