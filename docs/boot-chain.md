...
How the device boots from an SD card when one is present, and falls back to the
stock OS when it is not. The card this chain reaches for is
[the card](card.md); the hardware underneath is [hardware](hardware.md).
...
The preloader is always **the unit's own, patched in place**. Nothing else in
NAND is touched: `mtd1` (U-Boot + BL31), `mtd2` (kernel) and `mtd3` (rootfs) stay
stock, so removing the card boots stock exactly as before.
...
First written from the running stock OS over adb (2026-08-22): `flash_erase` and
`nandwrite` both `rc=0`, readback byte-exact, 0 ECC failures, 0 corrected bits,
0 bad blocks. Then from a card, unattended, on battery — see below.

Result — **all three cases correct**:
...
Pre-kernel **3.114 s**, against 3.118 s under GammaLoader's loader — no cost
either way; boot time was never the aim. DDR DVFS is healthy on the unit's own
V1.18 blob: all four FSPs (324/528/780/1056 MHz), `dmc_ondemand`, ATF version
`0x102`, and **no `loader&trust unmatch`**.
...
1. **Device still boots** (the expected failure: SD ignored, stock comes up) —
   rewrite the original. This is the case the design guarantees.
2. **Bootrom rejects the IDB** — it finds no valid preloader and enters USB
   **MASKROM**, recoverable with RKDevTool. Not a brick.
3. **Valid IDB that hangs** — the same, with MASKROM forced.

All three are in [recovery](recovery.md), which also covers taking the backup
this depends on.
...
`tools/mkpreloader.py`'s** — which `tests/test-preloader-patch.sh` checks on
every run, against a synthetic preloader, along with both tools failing closed on
a corrupt, wrong-sized or already-patched image.
